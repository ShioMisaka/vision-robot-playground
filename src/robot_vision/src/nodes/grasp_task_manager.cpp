...
void GraspTaskManager::step_approach() {
  ...
  LOG_INFO("Approaching: current [{:.4f}, {:.4f}, {:.4f}] -> approach [{:.4f}, {:.4f}, {:.4f}], "
  ...
  LOG_INFO("Approach done: actual [{:.4f}, {:.4f}, {:.4f}], position error {:.4f}m",
...
bool GraspTaskManager::step_redetect() {
  ...
  for (int retry = 0; retry < kMaxRetries; ++retry) {
    ...
    if (!base_xyz.has_value()) {
      LOG_WARN("Re-detection attempt {}/{} failed: TF lookup error",
               retry + 1, kMaxRetries);
      continue;
    }

    if (target_xyz_.has_value()) {
      LOG_INFO("Target updated: [{:.4f}, {:.4f}, {:.4f}] -> [{:.4f}, {:.4f}, {:.4f}]",
...
void GraspTaskManager::step_descend() {
  ...
  auto pose = robot_->get_end_effector_pose();
  LOG_INFO("Descending: current [{:.4f}, {:.4f}, {:.4f}] -> target [{:.4f}, {:.4f}, {:.4f}] "
  ...
  // 验证是否到达抓取高度（检测 FAULT/超时）
  {
    ...
    LOG_INFO("Descend done: actual [{:.4f}, {:.4f}, {:.4f}], position error {:.4f}m",
...
std::optional<std::array<double, 3>> GraspTaskManager::transform_to_base(
  ...
  if (log_details) {
    ...
    LOG_INFO("[TRANSFORM] Step 2 - hand t=[{:.4f}, {:.4f}, {:.4f}] rpy=[{:.4f}, {:.4f}, {:.4f}]",
             (*tf_hand)[0], (*tf_hand)[1], (*tf_hand)[2],
             (*tf_hand)[3], (*tf_hand)[4], (*tf_hand)[5]);
    LOG_INFO("[TRANSFORM] Step 3 - camera extrinsics: offset=[{:.4f}, {:.4f}, {:.4f}] "
             "rpy=[{:.4f}, {:.4f}, {:.4f}]",
  ...
  if (log_details) {
    ...
    LOG_INFO("[TRANSFORM] Step 4 - R columns (optical axes in base): "
             ...
             R(0,1), R(1,1), R(2,1),
             R(0,2), R(1,2), R(2,2));
    LOG_INFO("[TRANSFORM] Step 5 - optical origin in base (t)=[{:.4f}, {:.4f}, {:.4f}]",
             t.x(), t.y(), t.z());
