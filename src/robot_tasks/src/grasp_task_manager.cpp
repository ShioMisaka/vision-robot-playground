...
const char *state_name(GraspState s) {
  ...
  case GraspState::kLifting:
    return "LIFTING";
  case GraspState::kDone:
    return "DONE";
  case GraspState::kError:
    return "ERROR";
  }
  return "UNKNOWN";
}
...
GraspTaskManager::GraspTaskManager(
    ...
    std::shared_ptr<IVisionProcessor> vision, const std::string &base_frame,
    const std::string &camera_frame, double approach_height,
    double grasp_height_offset, const std::array<double, 3> &grasp_rpy,
    int redetect_samples, double redetect_interval, double max_reach,
    double approach_step_size, double approach_tolerance,
    int max_approach_steps, int max_consecutive_failures,
    const std::string &hand_frame, const std::array<double, 3> &camera_offset,
    const std::array<double, 3> &camera_rpy, double optical_frame_pitch)
    : robot_(std::move(robot)), vision_(std::move(vision)),
      base_frame_(base_frame), camera_frame_(camera_frame),
      approach_height_(approach_height),
      grasp_height_offset_(grasp_height_offset), grasp_rpy_(grasp_rpy),
      max_reach_(max_reach), approach_step_size_(approach_step_size),
      ...
      redetect_interval_(redetect_interval), hand_frame_(hand_frame),
      camera_offset_(camera_offset), camera_rpy_(camera_rpy),
      optical_frame_pitch_(optical_frame_pitch) {}
  ...
  try {
    while (state_ != GraspState::kDone && state_ != GraspState::kError) {
      ...
      if (abort_requested_.load(std::memory_order_acquire)) {
        LOG_WARN("Grasp aborted by external request");
      ...
      auto elapsed = std::chrono::duration<double>(
                         ...
                         .count();
      if (elapsed > timeout) {
        LOG_ERROR("Grasp timeout ({:.1f}s)", timeout);
        state_ = GraspState::kError;
        return false;
      }
    ...
    LOG_ERROR("Exception in state {}: {}", state_name(state_.load()), e.what());
    state_ = GraspState::kError;
    return false;
  }
...
bool GraspTaskManager::step_detect() {
  ...
  if (!base_xyz.has_value()) {
    LOG_WARN("TF lookup failed, cannot determine grasp target in base frame");
    return false;
  }

  target_xyz_ = *base_xyz;
  LOG_INFO("Detected target at [{:.4f}, {:.4f}, {:.4f}] -> APPROACHING",
...
void GraspTaskManager::step_approach() {
  ...
  if (dist > max_reach_) {
    LOG_ERROR("Approach target is {:.3f}m from base, exceeds max reach {:.2f}m",
              dist, max_reach_);
    state_ = GraspState::kError;
    return;
  }
  ...
  LOG_INFO("Approaching: current [{:.4f}, {:.4f}, {:.4f}] -> approach [{:.4f}, "
           "{:.4f}, {:.4f}], "
           "distance={:.3f}m (moveL, keep orientation)",
  ...
  try {
    ...
    LOG_ERROR("Approach moveL failed: {}", e.what());
    state_ = GraspState::kError;
    return;
  }
  ...
  LOG_INFO(
      "Approach done: actual [{:.4f}, {:.4f}, {:.4f}], position error {:.4f}m",
  ...
  if (pos_error > kApproachPosTolerance) {
    LOG_ERROR("Approach position error {:.4f}m exceeds tolerance {:.2f}m "
...
bool GraspTaskManager::step_redetect() {
  ...
  for (int retry = 0; retry < kMaxRetries; ++retry) {
    ...
    auto result =
        vision_->average_detections(redetect_samples_, redetect_interval_);

    if (!result.has_value() || !result->detected) {
      LOG_WARN("Re-detection attempt {}/{} failed: no valid detection",
               retry + 1, kMaxRetries);
      continue;
    }
    ...
    if (!base_xyz.has_value()) {
      LOG_WARN("Re-detection attempt {}/{} failed: TF lookup error", retry + 1,
               kMaxRetries);
      continue;
    }

    if (target_xyz_.has_value()) {
      LOG_INFO("Target updated: [{:.4f}, {:.4f}, {:.4f}] -> [{:.4f}, "
               "{:.4f}, {:.4f}]",
  ...
  LOG_ERROR("Re-detection failed after {} attempts", kMaxRetries);
  return false;
}
...
void GraspTaskManager::step_descend() {
  ...
  auto pose = robot_->get_end_effector_pose();
  LOG_INFO("Descending: current [{:.4f}, {:.4f}, {:.4f}] -> target [{:.4f}, "
           "{:.4f}, {:.4f}] "
           "(grasp height offset +{:.3f}m, keeping current orientation)",
           pose[0], pose[1], pose[2], target[0], target[1], target[2],
  ...
  try {
    ...
    LOG_ERROR("Descend moveL failed: {}", e.what());
    state_ = GraspState::kError;
    return;
  }
  ...
  // 验证是否到达抓取高度（检测 FAULT/超时）
  {
    ...
    LOG_INFO(
        "Descend done: actual [{:.4f}, {:.4f}, {:.4f}], position error {:.4f}m",
        actual.x(), actual.y(), actual.z(), pos_error);
    if (pos_error > 0.05) {
      LOG_ERROR("Descend position error {:.4f}m > 0.05m, aborting grasp",
                pos_error);
      state_ = GraspState::kError;
      return;
    }
...
void GraspTaskManager::step_lift() {
  ...
  LOG_INFO("Lifting {:.3f}m...", lift_height);

  try {
    robot_->move_linear({0.0, 0.0, lift_height}, "base", -1.0, true);
  } catch (const std::exception &e) {
    LOG_ERROR("Lift failed: {}", e.what());
    state_ = GraspState::kError;
    return;
  }
...
GraspTaskManager::transform_to_base(const Eigen::Vector3d &camera_xyz,
                                    bool log_details) {
  ...
  auto tf_hand = robot_->lookup_transform(base_frame_, hand_frame_, 1.0);
  if (!tf_hand.has_value()) {
    return std::nullopt;
  }

  if (log_details) {
    LOG_INFO("[TRANSFORM] Step 1 - camera_xyz=[{:.4f}, {:.4f}, {:.4f}]",
             camera_xyz.x(), camera_xyz.y(), camera_xyz.z());
    LOG_INFO("[TRANSFORM] Step 2 - hand t=[{:.4f}, {:.4f}, {:.4f}] "
             "rpy=[{:.4f}, {:.4f}, {:.4f}]",
             (*tf_hand)[0], (*tf_hand)[1], (*tf_hand)[2], (*tf_hand)[3],
             (*tf_hand)[4], (*tf_hand)[5]);
    LOG_INFO("[TRANSFORM] Step 3 - camera extrinsics: offset=[{:.4f}, {:.4f}, "
             "{:.4f}] "
             "rpy=[{:.4f}, {:.4f}, {:.4f}]",
  ...
  // 2. base ← hand
  Eigen::Matrix3d R_bh =
      rpy_to_rotation((*tf_hand)[3], (*tf_hand)[4], (*tf_hand)[5]);
  Eigen::Vector3d t_bh((*tf_hand)[0], (*tf_hand)[1], (*tf_hand)[2]);

  // 3. hand ← camera_link（已知相机外参）
  Eigen::Matrix3d R_hc =
      rpy_to_rotation(camera_rpy_[0], camera_rpy_[1], camera_rpy_[2]);
  Eigen::Vector3d t_hc(camera_offset_[0], camera_offset_[1], camera_offset_[2]);

  // 4. camera_link ← optical（USD 相机 → ROS 光学坐标系: Ry(π)）
  Eigen::Matrix3d R_co =
      rpy_to_rotation(0.0, optical_frame_pitch_, 0.0);
  ...
  if (log_details) {
    ...
    LOG_INFO("[TRANSFORM] Step 4 - R columns (optical axes in base): "
             "X=[{:.3f},{:.3f},{:.3f}] Y=[{:.3f},{:.3f},{:.3f}] "
             "Z=[{:.3f},{:.3f},{:.3f}]",
             R(0, 0), R(1, 0), R(2, 0), R(0, 1), R(1, 1), R(2, 1), R(0, 2),
             R(1, 2), R(2, 2));
    LOG_INFO("[TRANSFORM] Step 5 - optical origin in base (t)=[{:.4f}, {:.4f}, "
             "{:.4f}]",
             t.x(), t.y(), t.z());
    LOG_INFO("[TRANSFORM] Step 6 - RESULT: base_point=[{:.4f}, {:.4f}, {:.4f}]",
             base_point.x(), base_point.y(), base_point.z());
  }

  return std::array<double, 3>{base_point.x(), base_point.y(), base_point.z()};
}
